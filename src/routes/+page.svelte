                ...
                file
                    ...
                    .then(fileContent => {
                        ...
                        if (errorNode) {
                            ...
                            logs.forEach(log => {
                                csv += '\r\n';
                                csv += Array.from(log.children).map(logField => logField.textContent).join(',');
                            });
...
<Container sm>
    ...
    <Form on:submit={handleSubmit}>
        <FormGroup>
            <Label for="xle-file">Upload a .xle file of your Levelogger data to convert to a .csv of your log data:</Label>
            <Input
                bind:files
                id="xle-file"
                name="xle"
